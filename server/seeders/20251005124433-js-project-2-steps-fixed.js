...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    try {
      ...
      // Удаляем существующие шаги для этого проекта
...
5. Выведите значение в консоль для проверки
          ...
          validationRules: JSON.stringify([
            {
              ...
              message: "Функция и обработчик события должны быть реализованы"
            }
          ]),
          order: 2,
          project_id: project.id,
          ...
          validationRules: JSON.stringify([
            {
              ...
              rules: [
                ...
                },
                {
                  ...
                  eventType: "click",
                  expectChange: true,
                  condition: "document.getElementById('result').textContent.length > 0",
...
**Подсказка:** Используйте \`isNaN()\` для проверки, является ли значение числом.`,
          validationRules: JSON.stringify([
            {
              ...
              rules: [
                ...
                },
                {
                  ...
                  eventType: "click",
                  expectChange: true,
                  condition: "document.getElementById('result').textContent.includes('возраст') || document.getElementById('result').textContent.includes('Введите') || document.getElementById('result').textContent.includes('Пожалуйста')",
                  message: "При клике должен отображаться результат или сообщение об ошибке"
                }
              ],
              message: "Финальная проверка: все компоненты с валидацией должны работать"
            }
          ]),
          order: 4,
          project_id: project.id,
      ...
      await t.commit();
      console.log('✅ Исправленные шаги для проекта "Калькулятор возраста" успешно созданы');
    } catch (error) {
      await t.rollback();
      throw error;
    }
