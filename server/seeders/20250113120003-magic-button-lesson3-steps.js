...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    if (projects.length === 0) {
      throw new Error(
        'Проект "Урок 3: Стилизация кнопки" не найден. Запустите сначала сидер курса.'
      );
    }
    ...
    return queryInterface.bulkInsert("projectSteps", [
      ...
      {
        instructions:
          "Создайте CSS правило для .magic-button с градиентным фоном linear-gradient(135deg, #ff6b6b, #ffa500), белым цветом текста, без границы и padding 20px 40px.",
        order: 2,
        project_id: projectId,
        validationRules: JSON.stringify([
          {
      ...
      {
        instructions:
          "Дополните стили кнопки: font-size: 1.5rem, font-weight: bold, border-radius: 50px, cursor: pointer, margin: 20px и box-shadow: 0 10px 25px rgba(255, 107, 107, 0.4).",
        order: 3,
        project_id: projectId,
        validationRules: JSON.stringify([
          {
      ...
      {
        instructions:
          "Добавьте свойства transition: all 0.3s ease, text-transform: uppercase и letter-spacing: 1px к кнопке.",
        order: 4,
        project_id: projectId,
        validationRules: JSON.stringify([
          {
      ...
      {
        instructions:
          "Создайте CSS правило для .magic-button:hover с эффектами: transform: scale(1.1) rotate(2deg), box-shadow: 0 15px 35px rgba(255, 107, 107, 0.6) и измененным градиентом background: linear-gradient(135deg, #ff5252, #ff8f00).",
        order: 5,
        project_id: projectId,
        validationRules: JSON.stringify([
          {
      ...
      {
        instructions:
          "Добавьте CSS правило для .magic-button:active с эффектом при нажатии: transform: scale(0.95) rotate(-1deg).",
        order: 6,
        project_id: projectId,
        validationRules: JSON.stringify([
          {
