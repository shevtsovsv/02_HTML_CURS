...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    if (projects.length === 0) {
      throw new Error(
        'Проект "Урок 1: Базовая HTML структура" не найден. Запустите сначала сидер курса.'
      );
    }
    ...
    return queryInterface.bulkInsert("projectSteps", [
      {
        instructions:
          "Добавьте DOCTYPE и базовую структуру HTML документа с тегами html, head и body.",
        order: 1,
      ...
      {
        instructions:
          'В секции head добавьте мета-тег для кодировки UTF-8, viewport мета-тег для адаптивности и заголовок страницы "Волшебная кнопка".',
        order: 2,
        project_id: projectId,
        validationRules: JSON.stringify([
      ...
      {
        instructions:
          'В body создайте div-контейнер, добавьте заголовок h1 с текстом "Волшебная кнопка", параграф с описанием "Нажми на кнопку и увидишь магию!" и кнопку с текстом "✨ Магия! ✨".',
        order: 3,
        project_id: projectId,
        validationRules: JSON.stringify([
